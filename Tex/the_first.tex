...
This leads us to the question of how we can represent our Lambertian reflection as a PDF.
...
It's rotationally symmetric around the surface normal.
...
$$p{combined}(\omega) = w{light} \cdot p{light}(\omega) + w{cosine} \cdot p_{cosine}(\omega)$$

Where $w{light}$ and $w{cosine}$ are weights that sum to 1. For example, we might choose $w{light} = w{cosine} = 0.5$ to give equal importance to both strategies.
...
As you can see we have now arrived back at our original image before looking into importance sampling.

\subsection{Analysis of Results}

This does raise the point of why we bothered to go through all the maths, and so I will take this section to quantitatively demonstrate how much of a speed advantage we have just gained.
