...
\subsection{Monte Carlo Integration and Variance Reduction}

\subsubsection{The Problem of Noise and Slow Convergence}

In our current implementation, the rendered image exhibits significant noise, and convergence to a clear image is slow. This issue stems from the nature of our path tracing algorithm, where most rays bounce into areas of low illumination or darkness. Mathematically, we can express the radiance \( L \) at a point \( p \) in direction \( \omega \) as:
